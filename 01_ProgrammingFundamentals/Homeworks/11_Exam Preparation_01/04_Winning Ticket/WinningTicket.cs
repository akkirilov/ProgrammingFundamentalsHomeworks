...
namespace _04_Winning_Ticket
{
    public class WinningTicket
    {
        public static void Main()
        {
            ...
            {
                if (tickets[t].Length != 20)
                {
                    Console.WriteLine("invalid ticket");
                    continue;
                }

                string leftSide = tickets[t].Substring(0, 10);
                string rightSide = tickets[t].Substring(10);
                bool isWin = false;
                for (int s = 0; s < winningSymbols.Count; s++)
                {
                    ...
                    if (leftCount > 5 && rightCount > 5)
                    {
                        ...
                        {
                            Console.WriteLine($"ticket \"{tickets[t]}\" - {(leftCount < rightCount ? leftCount : rightCount)}{winningSymbols[s]}");
                            isWin = true;
                            break;
                        }
                        else if (leftCount == 10 && rightCount == 10)
                        {
                            Console.WriteLine($"ticket \"{tickets[t]}\" - {leftCount}{winningSymbols[s]} Jackpot!");
                            isWin = true;
                            break;
                        }
                    }
                ...
                if (!isWin)
                {
                    Console.WriteLine($"ticket \"{tickets[t]}\" - no match");
                }
            }
        }
    }
