	...
	public void run() throws IOException {
		...
		while (!line.equals("Peace")) {
			...
			case "createparticipant":
				...
				break;
			case "createaction":
				this.battleField.createAction(lineTokens[1], Arrays.copyOf(Arrays.stream(lineTokens).skip(2).toArray(),
						Arrays.stream(lineTokens).skip(2).toArray().length, String[].class));
				break;
			case "statactions":
