...
public class DragRace extends Race {
	...
	public String getWinners() {
		...
		LinkedHashMap<Integer, Car> orderedParticipants = super.getParticipants().entrySet().stream().sorted(
				(e1, e2) -> Integer.compare(e2.getValue().getEnginePerformance(), e1.getValue().getEnginePerformance()))
		...
		for (Map.Entry<Integer, Car> entry : orderedParticipants.entrySet()) {
			...
			sb.append(count).append(". ").append(entry.getValue().getBrand()).append(" ")
					.append(entry.getValue().getModel()).append(" ").append(entry.getValue().getEnginePerformance())
